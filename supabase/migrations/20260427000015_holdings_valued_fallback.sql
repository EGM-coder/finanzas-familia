-- Migración 15 — holdings_valued: fallback a avg_price_eur cuando no hay precio
-- Razón: holdings sin cotización pública (ej. roboadvisors agregados) deben
-- usar el valor manual cargado en avg_price_eur en lugar de NULL.
...
LEFT JOIN LATERAL (
  ...
  WHERE (ticker IS NOT DISTINCT FROM h.ticker)
    AND (isin IS NOT DISTINCT FROM h.isin)
  ORDER BY date DESC
  LIMIT 1
) hp ON TRUE;

-- Recrear account_balances_full (igual que migración 14)
CREATE VIEW public.account_balances_full
WITH (security_invoker = TRUE) AS
SELECT
  a.id, a.name, a.institution, a.type, a.visibility,
  a.linked_account_id, a.initial_balance,
...
-- Recrear patrimonio_neto (igual que migración 12)
