...
| T-037 | **RESUELTA** (mig-61, 28-jun-2026). Prorrateo mensual→semanal en `fn_close_week`: la semana ISO puede cruzar un mes (ej. 28-jun – 4-jul). La función usa `generate_series(week_start, week_end, '1 day')` agrupado por (year, month) para calcular los días de cada tramo, y multiplica `amount_planned × days_in_seg / days_in_month` por tramo antes de sumar. Aplica tanto al `total_budget` global como al presupuesto por categoría dentro de `top_deviations`. | — |
