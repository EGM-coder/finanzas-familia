...
| T-028 | **RESUELTA** (03-jun-2026). Nav de módulos MVP: sidebar 200px (desktop) + tab bar bottom (mobile) cableados al layout `(egm)`. Módulos activos: I·Inicio, II·Control, III·Pedidos, IV·Horizonte, V·Presupuesto. Módulos deshabilitados (span, sin href): VI·Maristas, VII·Asesor IA. Estado activo: `borderLeft: 2px solid var(--ink)` + `fontWeight 500`. `EgmTopBar` oculto en desktop (sidebar tiene branding); visible en mobile. Archivo: `EgmNav.tsx` + `layout.tsx`. | — |
